...
run:
	docker run -it --rm --name dockerbase-devbase-gvm $(NAME):$(VERSION)
...
version:
	...
	docker run -it --rm $(NAME):$(VERSION) bash -c " source /home/devbase/.gvm/bin/gvm-init.sh && gvm version " | tee -a COMPONENTS
	dos2unix COMPONENTS
